...
static int __init zili_demo_char_button_init(void)
{
	...
	cdev_init(&dev_button, &zili_demo_fops);
	dev_button.owner = THIS_MODULE;
	dev_button.ops   = &zili_demo_fops;
	ret = cdev_add(&dev_button, devno, N_D);
	if(ret)
	{
